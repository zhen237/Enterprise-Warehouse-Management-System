 ...
 * 提供系统数据的初始化与重置接口
 *
 * @api GET /api/data/init   初始化示例数据（供应商→商品→仓库→库存→入库→出库→盘点）
 * @api GET /api/data/reset 清空所有业务数据（按外键反向顺序删除）
 *
 * 数据依赖顺序：
 *   供应商 → 商品 → 仓库 → 库存 → 入库记录 → 出库记录 → 盘点记录
 */
@RestController
...
public class DataController {
    ...
    /**
     * 初始化示例数据
     * 按依赖顺序创建：供应商 → 商品 → 仓库 → 库存 → 入库记录 → 出库记录 → 盘点记录
     * 若数据库中已有数据则跳过初始化
     *
     * @return 初始化结果消息
     */
    ...
    /**
     * 清空所有业务数据
     * 按外键依赖反向顺序删除：盘点 → 出库 → 入库 → 库存 → 仓库 → 商品 → 供应商
     *
     * @return 清空结果消息
     */
    @GetMapping("/reset")
